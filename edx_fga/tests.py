...
@ddt
class FreeformGradedAssignmentXblockTests(ModuleStoreTestCase):
    """
    Create a fga block with mock data.
    """
    def setUp(self):
        """
    ...
    def make_one(self, display_name=None, **kw):
        """
        Creates a XBlock fga for testing purpose.
        """
        from edx_fga.fga import FreeformGradedAssignmentXBlock as cls
    ...
    def test_ctor(self):
        ...
        self.assertEqual(block.display_name, "Freeform Graded Assignment")
    ...
    @mock.patch('edx_fga.fga._resource', DummyResource)
    @mock.patch('edx_fga.fga.render_template')
    @mock.patch('edx_fga.fga.Fragment')
    def test_student_view(self, fragment, render_template):
        ...
        fragment.add_css.assert_called_once_with(
            DummyResource("static/css/edx_fga.css"))
        fragment.initialize_js.assert_called_once_with(
            "FreeformGradedAssignmentXBlock")
    ...
    def test_studio_view(self, fragment, render_template):
        ...
        self.assertEqual(tuple(context['fields']), (
            (cls.display_name, 'Freeform Graded Assignment', 'string'),
    ...
    def test_save_fga(self):
        """
        Tests save fga  block on studio.
        """
        def weights_positive_float_test():
            ...
            block.save_fga(mock.Mock(method="POST", body=json.dumps({
    ...
    def test_upload_download_assignment(self):
        ...
        with patch(
            "edx_fga.fga.FreeformGradedAssignmentXBlock._file_storage_path",
    ...
    def test_enter_grade_fail(self, grade):
        ...
        block = self.make_one()
        fred = self.make_student(block, "fred5", filename='foo.txt')
        with patch('edx_fga.fga.log') as mocked_log:
            block.enter_grade(mock.Mock(params={
                'module_id': fred['module'].id,
                'submission_id': fred['submission']['uuid'],
                'grade': grade}
